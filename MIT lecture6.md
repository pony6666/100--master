...
6/20/2019 7:26:20 PM 

最后把lecture6中的最后关于字典的小尾巴弄结束。

Python为集合提供了集中内置的数据类型。除了列表外，字典的集合类型可能是应用最广泛的使用。

映射：允许查找与任意键相关联的信息的集合称为映射。
...
1. 创建一个频率字典映射STR：INT
2. 找到出现频率最高的一个字，表示出频率
- 使用列表，防止有不止一个字的出现
- 对于（words_list，highest_freq）返回一个元组（list，int）
3. 找到出现至少X次的字
- 让用户选择	“at	least	X	times”,所以允许作为一个参数
- 返回一个元组列表，每个元组是一个（list，int），包含按频率排序的单词列表
- IDEA：从歌词典，找到频率最高的字。删除最常见的词。 重复。它起作用是因为你正在改变这首歌字典。

6/20/2019 8:01:00 PM 
...
还是从流程控制的角度来说明上面的代码：两个初始化数据

就一个大的while循环，条件是不是错误的，也就是正确的时候，执行下面的代码。然后返回一个结果

里面定义了一个变量，最高的频率的单词。

然后是一个if句，有两种情况，第一种情况里面又镶嵌一个for循环，目的是为了删除一些单词。

### 卡点上面的代码种的temp[1]是什么意思 ###

通过查找自己做的一个分类后的教程后，找到答案。只不过是索引而已，temp[1]代表是第二个位置的元素。