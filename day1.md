...
### day5 ###
5/23/2019 3:07:18 PM 

开始day5的学习
今天学习的内容是函数和模块的使用
一道数学题，里面的两张图片加载不出来，所以直接代码。
这道数学题更生活化的说法就是，将8个苹果分成四组每组至少一个苹果有多少种方案。
先8-4=4，然后就是4，‘3，1’，‘2，1，1’，‘2，2’，‘1，1，1，1’，我自己能想出五种
再打打代码吧
    `m=int(input('m= '))
	n=int(input('n= '))
	fm=1
...
### 卡点 ###
打出的这段代码，真不知道沈意思，先往下看。

函数的作用：
刚打出的没弄明白什么意思的代码，观察到一点就是有三个重复。

代码有很多种坏味道，重复是最坏的一种。

可以将一个功能分装到一个称之为‘函数’的功能模块中，用以解决重复的情况。

定义函数
在Python中可以用def关键字来定义函数，和变量一样每个函数也有一个想量的名字，而且明明规则更变量的明明规则是一致的。

编程与数学的一个类似点：在函数名后面的圆括号中可以放置传递给函数的参数，这一i但和数学上的函数非常相似，程序中函数的参数就相当于数学上说的函数的自变量，而函数之心给完成后我们可以通过return关键字来返回一个值，这相当于数学上说的函数的因变量。

把上面的功能封装成一个函数就是

先暂停一下，day5的学习，看看自学是门手艺
把编程作为自学的入口的原因
1. 它就是无论是谁都能学会的
2. 适合仅靠阅读自学，因为这个领域发展很快
3. 这个领域的知识结构最接近每个人所面对的人生中的知识结构
4. 能提高自己的英语阅读能力
5. 编程具是现实的，可行动的，并且还是真正可达成的
6. 在编程领域，自学的人最多


5/23/2019 5:07:35 PM 
